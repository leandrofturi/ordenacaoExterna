...
int lin_search(FILE **disp, int *idexes, int block_R, int P) {
    ...
    int tabu_disp[P], left_devices = P, read_devices = 0; // dispositivos tabu
    ...
    while(1 && j < 30) {
        for(r = 0; r < P; r++) {
            if(feof(disp[r+block_R])) { // fim do arquivo
                tabu_disp[r] = 1;
                left_devices--;
                read_devices++;
            }
...
void BMM(int M, int P, char *filename, int *idexes) {
    ...
    while(exit) {
        ...
        for(p = block; ; p = ((p+1)%P)+block) {
            ...
            if(feof(fileR)) {
                int j = 0;
                for(p = block ; j < P ; p = ((p+1)%P)+block ) {
                    fseek(disp[p], -2, SEEK_END);
                    fprintf(disp[p], "\n");
                    j++;
                }
                fclose(fileR);
